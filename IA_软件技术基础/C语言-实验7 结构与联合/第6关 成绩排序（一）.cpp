...
void sorts(struct Student **head) {
    ...
    do {
        ...
        while (current->next != end)
        {
            ...
            if (a_cur > avg_next)
            {
                char t_Stuid[20], t_Name[20];
                int t_Score[4];

                strcpy(t_Stuid, current->stuid);
                strcpy(t_Name, current->name);
                memcpy(t_Score, current->score, 4 * sizeof(int));

                strcpy(current->stuid, next->stuid);
                strcpy(current->name, next->name);
                memcpy(current->score, next->score, 4 * sizeof(int));

                strcpy(next->stuid, t_Stuid);
                strcpy(next->name, t_Name);
                memcpy(next->score, t_Score, 4 * sizeof(int));
