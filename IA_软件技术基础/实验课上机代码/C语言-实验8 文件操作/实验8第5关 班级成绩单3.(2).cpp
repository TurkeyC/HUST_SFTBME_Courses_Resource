...
/*用于将链表中的数据保存到文件中*/
void f_save(struct Student **head) {
    FILE *fp = fopen("stu.dat", "wb");
    struct Student *current = *head;
    while (current != NULL) {
        fwrite(current, sizeof(struct Student), 1, fp);
        current = current->next;
    }
    fclose(fp);
}
...
/*用于从文件中读取数据到链表中*/
void f_load(struct Student **head){
    FILE *fp = fopen("stu.dat", "rb");
    struct Student *current = *head;
    while (current != NULL) {
        fread(current, sizeof(struct Student), 1, fp);
        current = current->next;
    }
    fclose(fp);
}
...
swt fpt[] = {cirinput, output, change ,aver, outall, f_save, f_load};
